...
#
# Single image that serves BOTH transports of the scikit-bio VGI worker:
#   docker run ... IMG            -> HTTP server on $PORT (default, Fly.io / local)
#   docker run -i ... IMG stdio   -> stdio worker DuckDB spawns on-host
# See docker-entrypoint.sh. Deps install from PyPI (vgi-python / vgi-rpc /
# scikit-bio are all published) — no vendored checkouts, no pin rewriting.
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-scikit-bio
...
#           WAL-SQLite BoundStorage)
LABEL org.opencontainers.image.title="vgi-scikit-bio" \
      org.opencontainers.image.description="scikit-bio as a VGI worker for DuckDB/SQL (stdio + HTTP)" \
      org.opencontainers.image.source="${SOURCE_URL}" \
      org.opencontainers.image.version="${VERSION}" \
      org.opencontainers.image.revision="${GIT_COMMIT}" \
      org.opencontainers.image.licenses="MIT" \
      farm.query.vgi.volumes='[{"path":"/data","name":"vgi_scikit_bio_state","purpose":"state","shared":true}]' \
      farm.query.vgi.transports='["tcp","http"]'

ENV PYTHONUNBUFFERED=1 \
    PIP_NO_CACHE_DIR=1 \
    PIP_DISABLE_PIP_VERSION_CHECK=1 \
    # Shared framework state on the /data volume (see the label above).
    ...
    VGI_SCIKIT_BIO_GIT_COMMIT=${GIT_COMMIT} \
...
# version is read from vgi_scikit_bio/__init__.py by hatchling, so the wheel and
# the advertised implementation_version match the release with no .git present.
COPY pyproject.toml README.md LICENSE ./
COPY vgi_scikit_bio ./vgi_scikit_bio
