...
`vgi-scikit-bio` is developed and maintained by **[Query.Farm](https://query.farm)**.
It is a [VGI](https://query.farm) worker that exposes
[scikit-bio](https://scikit.bio) — sequence analysis, community diversity,
ordination, and phylogenetics — to DuckDB/SQL.
...
- a minimal SQL snippet that reproduces the problem (ideally using inline
  `VALUES` data);
