...
#
# Dispatch the single vgi-scikit-bio image into one of its transports:
...
# A freshly-mounted (empty) volume has no subdirs; create the state dir so the
# non-root user can write the WAL SQLite. Harmless when /data is unmounted.
mkdir -p "$(dirname "${VGI_WORKER_SQLITE_PATH:-/data/state/vgi_storage.db}")"

case "${1:-http}" in
  http)
    shift 2>/dev/null || true
    exec vgi-scikit-bio-http --host 0.0.0.0 --port "${PORT:-8000}" "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    ...
    exec vgi-scikit-bio --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec vgi-scikit-bio "$@"
