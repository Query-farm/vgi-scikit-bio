...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the scikit-bio
...
#   VGI_SCIKIT_BIO_WORKER worker LOCATION the .test files attach. One of:
#                           - a stdio command (e.g. the installed `vgi-scikit-bio`)
#                           - `launch:<command>` for the warm Unix-socket launcher
#                             transport (worker spawned once, reused per ATTACH —
#                             avoids per-attach Python/scikit-bio import startup)
...
: "${HAYBARN_UNITTEST:?path to the haybarn-unittest binary}"
: "${VGI_SCIKIT_BIO_WORKER:?worker LOCATION (stdio command or http:// URL)}"
...
echo "Running suite (pattern: $TEST_PATTERN, worker: $VGI_SCIKIT_BIO_WORKER) ..."
