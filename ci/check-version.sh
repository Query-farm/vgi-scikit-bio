...
INIT="$HERE/../vgi_scikit_bio/__init__.py"
...
if [ "$TAG" != "$VERSION" ]; then
  echo "::error::release tag ($TAG) does not match package __version__ ($VERSION); bump vgi_scikit_bio/__init__.py before tagging." >&2
  exit 1
fi
