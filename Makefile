# scikit-bio VGI worker — dev, test, and deploy targets.
#
# Usage:
#   make test              # pytest unit + SQL (stdio/http)
...
WORKER_STDIO   ?= uv run --python 3.13 scikit_bio_worker.py
WORKER_HTTP    ?= http://localhost:8000
WORKER_CLOUD   ?= https://$(FLY_APP).fly.dev
HTTP_PORT      ?= 8000

# Fly.io config
FLY_APP        ?= vgi-scikit-bio

.PHONY: test pytest test-stdio test-http test-docker-stdio test-docker-http \
        test-cloud image smoke-test deploy venv
...
venv:
	...
	uv pip install --python .venv \
		...
		"scikit-bio>=0.6" numpy pandas scipy pytest
...
test-stdio:
	...
	uv run --reinstall-package vgi-python --python 3.13 scikit_bio_worker.py --help >/dev/null 2>&1 || true
	VGI_SCIKIT_BIO_WORKER="$(WORKER_STDIO)" \
...
test-http:
	...
	@VGI_SIGNING_KEY=dev .venv/bin/python serve.py --port $(HTTP_PORT) & \
		...
		VGI_SCIKIT_BIO_WORKER="$(WORKER_HTTP)" $(TEST_RUNNER) --test-dir "$(TEST_DIR)" "$(TEST_PATTERN)"; \
		TEST_EXIT=$$?; \
...
test-cloud:
	VGI_SCIKIT_BIO_WORKER="$(WORKER_CLOUD)" $(TEST_RUNNER) --test-dir "$(TEST_DIR)" "$(TEST_PATTERN)"
...
VERSION        := $(shell sed -nE 's/^__version__ = "([^"]+)".*/\1/p' vgi_scikit_bio/__init__.py)

GHCR_IMAGE     ?= ghcr.io/query-farm/vgi-scikit-bio
...
DOCKER_IMAGE     ?= vgi-scikit-bio:dev
DOCKER_STATE_VOL ?= vgi_scikit_bio_state_test
...
test-docker-stdio: image
	...
	VGI_SCIKIT_BIO_WORKER="docker run -i --rm -v $(DOCKER_STATE_VOL):/data $(DOCKER_IMAGE) stdio" \
		$(TEST_RUNNER) --test-dir "$(TEST_DIR)" "$(TEST_PATTERN)"
	-docker volume rm $(DOCKER_STATE_VOL) >/dev/null 2>&1
...
# Quick local probe of the built image's HTTP mode.
smoke-test: image
